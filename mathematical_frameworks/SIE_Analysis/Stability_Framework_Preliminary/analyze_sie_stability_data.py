...
def analyze_sweep_results(results_dir):
    ...
    sweep_summary.sort(key=lambda x: x['params']['lambda_decay'])

    print("\n\n--- Parameter Sweep Summary ---")
    print("Lambda Decay | Final ||W|| | Reward Mean | Reward Std | V(state) Converged | V(state) Final Std")
    print("------------|-------------|-------------|------------|--------------------|-------------------")
    for result in sweep_summary:
        ...
        print(f"{lam:<12.4f} | {final_norm:<11.4f} | {r_mean:<11.4f} | {r_std:<10.4f} | {str(v_conv):<18} | {v_std:<17.4f}")
